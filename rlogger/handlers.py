...
class RLoggerNativeHandler(logging.handlers.SocketHandler):
    ...
    def __init__(self, tag, host='', port=10382, socket_path=None, chunk_size=8388608):
    ...
    def sbuf_gen(self, t, s):
        b = memoryview(s.encode(sys.getdefaultencoding()))
        sp = 0
        while True:
            ...
            if n == 0:
                break
            yield struct.pack(RLoggerNativeHandler.SBUF_PACK_FMT + str(n) + 's', int(t), n, v.tobytes())
            sp += n
